...
def test_create_product_duplicate_name(authenticated_client, user_factory):
    ...
    authenticated_client.post(
        "/api/v1/products/", {"name": "Unique", "price": "10"}, format="json"
    )
    response = authenticated_client.post(
        "/api/v1/products/", {"name": "Unique", "price": "20"}, format="json"
    )
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_product_detail(authenticated_client, user):
    ...
    create_resp = authenticated_client.post(
        "/api/v1/products/",
        {"name": "DetailTest", "price": "15", "category_id": category.id},
        format="json",
    )
...
def test_update_product(authenticated_client, user):
    create_resp = authenticated_client.post(
        "/api/v1/products/", {"name": "EditMe", "price": "50"}, format="json"
    )
    product_id = create_resp.json()["data"]["id"]
    response = authenticated_client.put(
        f"/api/v1/products/{product_id}/",
        {"name": "Edited", "price": "75"},
        format="json",
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_partial_update_product(authenticated_client, user):
    create_resp = authenticated_client.post(
        "/api/v1/products/", {"name": "PartialMe", "price": "100"}, format="json"
    )
    product_id = create_resp.json()["data"]["id"]
    response = authenticated_client.patch(
        f"/api/v1/products/{product_id}/", {"name": "PartiallyUpdated"}, format="json"
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_delete_product(authenticated_client, user):
    create_resp = authenticated_client.post(
        "/api/v1/products/", {"name": "DeleteMe", "price": "30"}, format="json"
    )
    product_id = create_resp.json()["data"]["id"]
...
def test_create_category_as_staff(api_client, admin_user):
    ...
    response = api_client.post(
        "/api/v1/products/categories/", {"name": "StaffCategory"}, format="json"
    )
    assert response.status_code == status.HTTP_201_CREATED
...
def test_create_category_as_non_staff(api_client, user):
    ...
    response = api_client.post(
        "/api/v1/products/categories/", {"name": "NoPerm"}, format="json"
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_update_category_as_staff(api_client, admin_user):
    ...
    api_client.force_authenticate(user=admin_user)
    response = api_client.put(
        f"/api/v1/products/categories/{cat.pk}/", {"name": "NewName"}, format="json"
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_filter_products_by_category(authenticated_client, user):
    ...
    authenticated_client.post(
        "/api/v1/products/",
        {"name": "ProdA", "price": "10", "category_id": cat_a.id},
        format="json",
    )
    authenticated_client.post(
        "/api/v1/products/",
        {"name": "ProdB", "price": "20", "category_id": cat_b.id},
        format="json",
    )
...
def test_search_products(authenticated_client, user):
    authenticated_client.post(
        "/api/v1/products/", {"name": "Laptop Pro", "price": "1500"}, format="json"
    )
    authenticated_client.post(
        "/api/v1/products/", {"name": "Mouse Pad", "price": "20"}, format="json"
    )
...
def test_order_products_by_price(authenticated_client, user):
    authenticated_client.post(
        "/api/v1/products/", {"name": "Cheap", "price": "5"}, format="json"
    )
    authenticated_client.post(
        "/api/v1/products/", {"name": "Expensive", "price": "100"}, format="json"
    )
